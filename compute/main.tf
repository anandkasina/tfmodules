# ----------Instance Template creation for compute ---------#  
locals {
...
locals {
  ...
  project_id = length(regexall("/projects/([^/]*)", google_compute_instance_template.tpl.self_link)) > 0 ? flatten(regexall("/projects/([^/]*)", google_compute_instance_template.tpl.self_link))[0] : null
